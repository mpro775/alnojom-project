...
function isSameSessionScope(a: AdminSession, b: AdminSession): boolean {
...
export async function adminRequestJson<T>(input: AdminRequestInput): Promise<T | null> {
...
async function resolveResponseData<T>(
  response: Response,
  responseType: NonNullable<AdminRequestOptions['responseType']>,
): Promise<T | null> {
...
function shouldRefresh(status: number, options: AdminRequestOptions): boolean {
  const requiresAuth = options.requiresAuth ?? true;
