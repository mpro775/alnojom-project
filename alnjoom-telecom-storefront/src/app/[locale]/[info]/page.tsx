...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; info: string }> }): Promise<Metadata> {
  const { locale, info: slug } = await params;
  const alternates = isLocale(locale) ? localizedAlternates(await getStoreConfigSafe(), locale, `/${slug}`) : undefined;
  return { title: slug, robots: { index: slug === "about" || slug === "contact" || slug === "shipping" || slug === "payment", follow: true }, alternates };
}
